...
const unsigned long TV_OFF_DEADBAND = 30000; // 30 seconds wait time after TV goes off before telling PI to power down
...
const char* stateNames[] = { IDNAME(INIT), IDNAME(TV_OFF), IDNAME(TV_ON), IDNAME(POWER_ON_PI), IDNAME(POWER_ON_PI_WAIT), IDNAME(POWER_OFF_PI), IDNAME(POWER_OFF_PI_WAIT) };
...
void loop() {
  ...
  switch (state) {
    ...
    case (POWER_ON_PI_WAIT):
      ...
      waitTime -= LOOP_DELAY;
      if (waitTime <= 0 && tvState == TV_IS_ON) {
        newState = TV_ON;
        delayTime = 10;
      }
      if (waitTime <= 0 && tvState == TV_IS_OFF) {
    ...
    case (TV_ON):
      if (tvState == TV_IS_OFF) {
        waitTime = TV_OFF_DEADBAND;
        do {
          delay(LOOP_DELAY);
          waitTime -= LOOP_DELAY;
          tvState = digitalRead(VSENSE_IN);
          Serial.print("State is TV_IS_OFF and waiting to see if TV comes back on: TV is ");
          Serial.println(tvState);
        } while (waitTime > 0 && tvState == TV_IS_OFF);
    ...
    case (POWER_OFF_PI_WAIT):
      ...
      waitTime -= LOOP_DELAY;
      if (waitTime <= 0) {
        newState = TV_OFF; //Regardless of whether TV has come on or not, power off the PI as the PI Will have shutdown
        delayTime = 10;
      }
      break;
    case (TV_OFF):
